...
2. 不得输出 schema 以外的字段。
3. `equipment.weapons` 和 `equipment.armor` 不能同时为空。
4. 所有装备模板 `id` 在整个装备包内必须唯一。
5. 同一件装备内的 `modifierContributions[].id` 必须唯一。
6. 所有数字字段必须是 JSON number，不能写成字符串。
7. 所有枚举字段必须使用合法值。
8. 护甲的 `baseThresholds.major` 必须大于或等于 `baseThresholds.minor`。
9. `definition.kind` 必须固定为 `modifier`。
10. 不确定内容必须报告，不要静默杜撰。
    ...
    "equipment": {
      ...
      "properties": {
        "weapons": {
          "type": "array",
          "items": {
            "$ref": "#/$defs/weaponTemplate"
          }
        },
        "armor": {
          "type": "array",
          "items": {
            "$ref": "#/$defs/armorTemplate"
          }
        }
      }
    },

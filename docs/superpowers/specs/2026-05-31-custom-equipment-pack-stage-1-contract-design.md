...
- Stage Commit Data：在内存中构造 next pack/index/state，但不写 storage 或 store state。
- Storage Transaction：保证提交成功或完整回滚。
- Registry Rebuild：只在 storage transaction 成功后重建 runtime registry/query index。
...
- adapter 按枚举字段值逐个判断：英文 canonical 保持不变，白名单中文 alias 转换为英文 canonical。
- 同一个装备包中英文枚举值混用是允许的；normalize 后必须全部变成英文 canonical。
- storage 不保存中文枚举 alias。
- registry/query 不返回中文枚举 alias。
...
### Diagnostic Code 固定为第一版 Union

第一版 `EquipmentPackImportDiagnostic.code` 不使用任意字符串，而是固定为 union type。后续新增 code 必须显式设计。

```ts
type EquipmentPackImportDiagnosticCode =
  | "INVALID_JSON"
  | "INVALID_FORMAT"
  | "MISSING_FIELD"
  | "UNKNOWN_FIELD"
...
interface EquipmentPackImportDiagnostic {
  severity: "error" | "warning"
  code: EquipmentPackImportDiagnosticCode
  path: string
  message: string
  value?: unknown
}
...
- `format` 缺失或不等于 `daggerheart.equipment-pack.v1` -> `INVALID_FORMAT`。
...
- `maxItems` 失败 -> `TEMPLATE_LIMIT_EXCEEDED` 或按字段映射到更具体容量 code。
...
type WeaponBurden = "oneHanded" | "twoHanded"
```
...
```json
{
  ...
  "weaponBurden": { "enum": ["oneHanded", "twoHanded"] },
...
- zh-CN alias adapter 在 core schema 前执行：`trait: "敏捷"`、`damageType: "物理"`、`range: "近战"`、`burden: "单手"` 能转换为英文 canonical 并通过 core schema。
...
- JSON parse 失败 -> `INVALID_JSON`。
- `format` 缺失或 const 不匹配 -> `INVALID_FORMAT`。
...
- `maxItems`：顶层 `weapons`/`armor` -> `TEMPLATE_LIMIT_EXCEEDED`。
...
- Semantic Validation 失败后不进入 Conflict Check 或 Stage Commit Data。
...
- `REGISTRY_REBUILD_FAILED`：Registry Rebuild，阶段 3。
- 真实 storage 回滚、registry 不重建、store state 不变：阶段 3 集成测试。
- UI 文案、展示顺序、多错误聚合策略。

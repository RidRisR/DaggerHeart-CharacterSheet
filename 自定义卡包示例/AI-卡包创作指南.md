...
2.  **严格遵守规范 (Strict Adherence to Specification)**：将提取的信息精确映射到正确的JSON字段，并遵守所有数据类型、枚举值和引用约束。**这是最高优先级原则，当其与"忠于原文"原则冲突时，必须优先遵守本规范，并根据第六部分的指引报告修正操作。**
...
| `简略信息`| Object | ❌ | 一个用于快速预览的键值对对象。键名应为 `item1`, `item2`, `item3`... 以此类推，用于展示卡牌最核心的几个特点，内容应保持简短。**不超过3个条目**。 |

...
- `main` 分支当前已发布版本产生的存档。
- 更早、没有 schema version 的旧存档，它们会先按既有迁移流程升级到 `v1`。
- 当前 modifier 分支最终落地后的 `v2` 存档。

不需要兼容 modifier 分支开发过程中出现过的中间态存档。原因是这些状态从未对外发布，只存在于本地开发 / 测试过程；为它们保留兼容逻辑会让 v1 -> v2 迁移和运行时代码承担不必要的复杂度。

因此 v2 迁移可以重写当前分支已经做过但尚未发布的 modifier 迁移逻辑，只要满足两个要求：

- 从已发布 `main` 存档迁移到 v2 不回归。
- 从当前最终 v2 存档再次加载时保持幂等，不重复创建 special contribution。

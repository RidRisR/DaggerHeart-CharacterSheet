...
interface HitPointsSectionProps {
  ...
  handleMaxChange: (field: keyof CharacterFormData, value: string) => void
  renderBoxes: (field: keyof CharacterFormData, max: number, total: number) => React.ReactElement
}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center">
            ...
            <input
              ...
              onChange={(e) => handleMaxChange("hpMax" as keyof CharacterFormData, e.target.value)}
              className="w-8 text-center border border-gray-400 rounded text-xs print:hidden" // 打印时隐藏
            />
          </div>
        </div>
        {renderBoxes("hp" as keyof CharacterFormData, Number(formData.hpMax || 6), 18)}
        ...
        <div className="flex items-center justify-between mt-1">
          ...
          <div className="flex items-center">
            ...
            <input
              ...
              onChange={(e) => handleMaxChange("stressMax" as keyof CharacterFormData, e.target.value)}
              className="w-8 text-center border border-gray-400 rounded text-xs print:hidden" // 打印时隐藏
            />
          </div>
        </div>
        {renderBoxes("stress" as keyof CharacterFormData, Number(formData.stressMax || 6), 18)}
      </div>
    </div>
  )
}
